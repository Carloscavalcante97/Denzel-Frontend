...
export default function ModalAdicionarMaterial({ onClose, onSelecionar, idEvento }: Props) {
  ...
  const buscarMateriais = async (termo: string) => {
    try {
      const url = `https://denzelbackend-f5faejb8a3akg9fu.brazilsouth-01.azurewebsites.net/api/materiais/Listar-por-nome-status?status=${encodeURIComponent("em estoque")}&nome=${encodeURIComponent(termo)}`;
        ...
        <div className="flex justify-end gap-4 mt-auto pt-4 border-t border-[#292343]">
          ...
          <button
            onClick={async () => {
              try {
                const id = idEvento;
                const response = await fetch(`https://denzelbackend-f5faejb8a3akg9fu.brazilsouth-01.azurewebsites.net/api/eventos/editarMaterial/${id}`, {
                  method: "PUT",
                  headers: {
                    "Content-Type": "application/json",
                    Authorization: `Bearer ${localStorage.getItem("token")}`,
