...
export default function ModalExcluirCliente({
  ...
  async function excluirCliente() {
    ...
    try {
      ...
      const response = await fetch(`https://denzelbackend-f5faejb8a3akg9fu.brazilsouth-01.azurewebsites.net/api/eventos/Deletar/${idEvento}`, {
        method: "DELETE",
        headers: {
      ...
      <div className="fixed inset-0 flex items-center justify-center p-4">
        <Dialog.Panel className="w-full max-w-md rounded-lg bg-[#1D1933] p-8 text-center shadow-lg">
          ...
          <Dialog.Description className="mt-4 text-white">
            O evento <span className="font-bold">{nomeEvento}</span>
          </Dialog.Description>
