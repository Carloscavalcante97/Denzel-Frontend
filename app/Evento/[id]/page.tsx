...
const EventoDetalhes = () => {
  ...
  const fetchResponsavel = async (idResponsavel: number) => {
    try {
      const response = await fetch(`http://localhost:3000/api/usuarios/buscar/${idResponsavel}`, {
        method: "GET",
        headers: {
  ...
  const fetchEvento = async () => {
    try {
      setLoading(true);
      const response = await fetch(`http://localhost:3000/api/eventos/Listar-Evento/${id}`, {
        method: "GET",
        headers: {
          "Content-Type": "application/json",
          Authorization: `Bearer ${localStorage.getItem("token")}`,
  ...
  const fetchCliente = async (idCliente: number) => {
    try {
      const response = await fetch(`http://localhost:3000/api/eventos/cliente/${idCliente}`, {
        method: "GET",
        headers: {
          "Content-Type": "application/json",
          Authorization: `Bearer ${localStorage.getItem("token")}`,
  ...
  const excluirMaterial = async (idMaterial: number) => {
    try {
      const response = await fetch(
        `http://localhost:3000/api/eventos/removerMaterial/${idMaterial}?eventoId=${evento?.Id}`,
        {
          method: "POST",
          headers: {
            Authorization: `Bearer ${localStorage.getItem("token")}`,
